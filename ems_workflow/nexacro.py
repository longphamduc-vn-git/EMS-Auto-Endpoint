...
def nexacro_xml_to_json(xml_text: str) -> Dict[str, Any]:
    ...
    có thể dùng JSONPath như $.reponse.itemName[*] truy vấn.
    ...
    for ds in root.findall(".//Dataset"):
        ...
        for row in ds.findall(".//Rows/Row"):
            ...
            for col in row.findall("Col"):
                ...
                row_data[key] = val
                # Tích lũy vào reponse dưới dạng columnar (list per field)
                output["reponse"].setdefault(key, []).append(val)
            if row_data:
